...
local function stage10ShortcutKind(entry)
...
local function stage10ManagerShouldShow()
    return Stage10.ManagerOpen and Gui.Main and Gui.Main.Visible
end

local function stage10RefreshManager()
...
local function stage10ToggleManager()
    if not Gui.Main then return end
...
local function stage10InstallOpenIcon()
...
local function stage11StopAllAnimationTracks(reason)
    refreshCharacterReferences()
    local hum = State.Character.Humanoid
    local animator = State.Character.Animator
    local tracks = {}
...
local function stage11FindOrCreateFolder(animate, stateName, folderName)
    if not animate then return nil end
...
local function stage11ClearAnimations(folder)
...
local function stage11AddWeight(anim, weights, fallbackWeight)
    local used = false
...
local function stage11CanonicalName(stateName, folderName, index, sourceName)
...
local function stage11GroupCurrentMappings()
    local groups = {}
...
local function stage11BuildGroupsFromCurrentForm()
    local groups = {}
    for _, stateName in ipairs(STATES) do
...
local function stage11ForceApplyGroups(groups)
    refreshCharacterReferences()
    local animate = State.Character.Animate
...
local function stage11StrictApplyCurrent(reason)
...
---------------------------------------------------------------------
-- CONTINUATION STAGE 12: 7YD7-STYLE APPLY, SAFER TIMING, CLEAN CARD X
-- Fixes current report without rewrite:
-- - No instant 0.25 respawn apply; wait until Animate/rig is actually ready.
-- - Bundle apply uses Animate-only by default, avoids HumanoidDescription mixing.
-- - No post-restart StopAllTracks that can fight Animate.
-- - Card layout cleaned: no LIST button over text; B add shortcut, X delete shortcut.
---------------------------------------------------------------------

local Stage12 = {
    Version = "12-safe-apply-clean-card",
}

-- Animation bundles should use Animate replacement by default. Description/Both can reset avatar in some games.
...
local function stage12RigReady(timeout)
    timeout = tonumber(timeout) or 2.5
    local started = os.clock()
    repeat
        refreshCharacterReferences()
        local char, hum, root, animate = State.Character.Instance, State.Character.Humanoid, State.Character.Instance and State.Character.Instance:FindFirstChild("HumanoidRootPart"), State.Character.Animate
        if char and hum and hum.Health > 0 and root and animate then
            local found = 0
            for _, n in ipairs({"idle","walk","run","jump","fall"}) do
                if animate:FindFirstChild(n) then found = found + 1 end
            end
...
local function stage12StopTracksBeforeApply()
...
local function stage12RestartAnimateOnly()
    refreshCharacterReferences()
    local animate = State.Character.Animate
    if animate then
        pcall(function()
...
local function stage12ClearFolder(folder)
    for _, child in ipairs(folder:GetChildren()) do
        if child:IsA("Animation") then child:Destroy() end
    end
end

local function stage12Folder(animate, folderName)
    if not animate then return nil end
    local lower = string.lower(tostring(folderName or ""))
    for _, child in ipairs(animate:GetChildren()) do
        if string.lower(child.Name) == lower then return child end
    end
...
local function stage12AnimName(folderName, sourceName, index)
    local folder = string.lower(tostring(folderName or ""))
    local source = tostring(sourceName or "")
    local lowerSource = string.lower(source)
    if folder == "idle" then
        if lowerSource:find("animation", 1, true) then return source end
        return "Animation" .. tostring(index)
    elseif folder == "walk" then return "WalkAnim"
    elseif folder == "run" then return "RunAnim"
    elseif folder == "jump" then return "JumpAnim"
    elseif folder == "fall" then return "FallAnim"
    elseif folder == "climb" then return "ClimbAnim"
    elseif folder == "swim" then return "Swim"
    elseif folder == "swimidle" then return "SwimIdle" end
    return source ~= "" and source or "Animation" .. tostring(index)
end

local function stage12ApplyWeights(anim, weights, fallback)
    local used = false
    if type(weights) == "table" then
        for _, val in ipairs(weights) do
...
local function stage12GroupsFromMappingsOrForm()
    local groups = {}
    if type(State.CurrentMappings) == "table" and #State.CurrentMappings > 0 then
        for _, m in ipairs(State.CurrentMappings) do
            local id = normalizeId(m.Id)
            local folder = tostring(m.Folder or "")
            local state = m.State
            if folder == "" and state then folder = (ANIMATE_NAMES[state] and ANIMATE_NAMES[state][1]) or string.lower(state) end
            if id ~= "" and folder ~= "" then
                local key = string.lower(folder)
                groups[key] = groups[key] or {Folder=folder, State=state, Items={}}
                table.insert(groups[key].Items, m)
            end
        end
    end
    ...
    if any then return groups end

    for _, state in ipairs(STATES) do
        local id = normalizeId(State.CurrentForm[state])
        if id ~= "" then
            local folder = (ANIMATE_NAMES[state] and ANIMATE_NAMES[state][1]) or string.lower(state)
            groups[folder] = {Folder=folder, State=state, Items={{Id=id, Name=stage12AnimName(folder, "", 1), Weights={}}}}
        end
    end
    return groups
end

local function stage12ApplyAnimateStrict(reason)
    if not stage12RigReady(2.5) then
        setStatus("Rig/Animate not ready", false)
        return false
    end
    refreshCharacterReferences()
    local animate = State.Character.Animate
    ...
    stage12StopTracksBeforeApply()

    local groups = stage12GroupsFromMappingsOrForm()
    local changed, folders = 0, 0
    for _, group in pairs(groups) do
        if group.Items and #group.Items > 0 then
            local folder = stage12Folder(animate, group.Folder)
            if folder then
                folders = folders + 1
                -- 7yd7-style: target folder is authoritative, no old/original Animation children left there.
                stage12ClearFolder(folder)
                for index, m in ipairs(group.Items) do
                    local id = normalizeId(m.Id)
                    if id ~= "" then
                        local anim = Instance.new("Animation")
                        anim.Name = stage12AnimName(group.Folder, m.Name, index)
                        anim.AnimationId = toAnimationUrl(id)
                        stage12ApplyWeights(anim, m.Weights, (string.lower(group.Folder) == "idle" and #group.Items > 1) and 10 or nil)
                        anim.Parent = folder
                        changed = changed + 1
                    end
                end
    ...
    if changed <= 0 then
        setStatus("No bundle animations to apply", false)
        return false
    end

    stage12RestartAnimateOnly()
    task.delay(0.25, function()
        -- One more restart after Animate has seen the new child list. Do NOT stop tracks after this.
        stage12RestartAnimateOnly()
    end)
    if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
    setStatus(tostring(reason or "Applied") .. " | clean replaced " .. changed .. " anims / " .. folders .. " folders", true)
    return true
end

applyCurrentForm = function(name)
    if name then State.LastAppliedName = name end
    local ok = stage12ApplyAnimateStrict(name or State.LastAppliedName or "Apply")
    if ok then saveData(); return end
    -- If strict failed, do a single-ID animate fallback only. No Description fallback here.
    for _, state in ipairs(STATES) do
        if normalizeId(State.CurrentForm[state]) ~= "" then setStateAnimation(state, State.CurrentForm[state]) end
    end
    stage12RestartAnimateOnly()
    saveData()
end
...
applyBundleFull = function(bundleId, bundleName)
    bundleId = normalizeId(bundleId)
    if bundleId == "" then notify("Invalid bundle", false); return end
    showLoading("Resolving bundle cleanly...")
    task.spawn(function()
        local mappings = stage9ResolveBundleMappings(bundleId)
        hideLoading()
        if not mappings or #mappings == 0 then
        ...
        saveData()
        if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
        local ok = stage12ApplyAnimateStrict(State.LastAppliedName)
        if ok then
            local idleCount = counts.Idle or 0
            notify("Bundle applied clean" .. (idleCount > 1 and (" + " .. idleCount .. " idle variants") or ""), true)
        end
    end)
end

local function stage12FloatingIndex(kind, id)
    id = tostring(id or "")
    for i, entry in ipairs(State.FloatingButtons) do
        if kind == "Emote" and entry.kind == "Emote" and tostring(entry.id) == id then return i end
        if kind == "Bundle" and entry.kind == "Bundle" and tostring(entry.bundleId) == id then return i end
    end
    return nil
end

local function stage12RemoveFloating(kind, id)
    local i = stage12FloatingIndex(kind, id)
    if not i then notify("No floating shortcut for this " .. string.lower(kind), false); return end
    table.remove(State.FloatingButtons, i)
    saveData(); rebuildFloatingButtons(); notify("Removed " .. kind .. " shortcut", true)
end

-- Clean final card renderer: no LIST overlay in the middle. B adds shortcut, X deletes shortcut next to it.
renderItemCard = function(parent, item, index, kind)
    local id = tostring(item.id or item.Id or "")
    local name = tostring(item.name or item.Name or (kind .. " " .. id))
    local creator = tostring(item.creatorName or item.CreatorName or "Unknown")
    ...
    button(card, kind == "Emote" and (State.Playback.Emote and tostring(State.Playback.Emote.id)==id and "STOP" or "PLAY") or (State.ChoosingState and "SET" or "APPLY"), UDim2.new(0,8,1,-38), UDim2.new(0,58,0,28), function()
        if kind == "Emote" then playEmote({id=id,name=name})
        elseif State.ChoosingState then setCustomSlotFromBundle(State.ChoosingState,id,name)
        else applyBundleFull(id,name) end
    end, kind == "Emote" and THEME.Green or THEME.Orange)
    button(card, "INFO", UDim2.new(0,71,1,-38), UDim2.new(0,48,0,28), function()
        if kind == "Emote" then
            showLoading("Resolving emote...")
            task.spawn(function()
                local real = resolveEmoteAnimation(id)
                local details = fetchAssetDetails(id) or {}
                hideLoading()
                local desc = tostring(details.description or details.Description or item.description or "No description available.")
                local body = "Name: "..name.."\nCreator: "..creator.."\nCatalog ID: "..id.."\nAnimation ID: "..tostring(real).."\n\n"..desc
                showInfoModal(name, body, imageId, {
                    {Text="PLAY", Color=THEME.Green, Callback=function() playEmote({id=id,name=name}) end, Close=false, W=70},
                    {Text="COPY", Color=THEME.Cyan, Callback=function() copyToClipboard(real or id) end, Close=false, W=70},
                    {Text="FLOAT", Color=THEME.Orange, Callback=function() createFloatingButton(item) end, Close=false, W=76},
            ...
            local body = "Name: "..name.."\nCreator: "..creator.."\nBundle ID: "..id.."\n\nAPPLY = full clean bundle apply. B = add floating. X = remove floating."
            showInfoModal(name, body, imageId, {
                {Text="APPLY", Color=THEME.Green, Callback=function() applyBundleFull(id,name) end, W=78},
    ...
    button(card, "X", UDim2.new(0,190,1,-38), UDim2.new(0,28,0,28), function()
        stage12RemoveFloating(kind, id)
    end, THEME.Red)
end

-- Safer lifecycle: fast enough, but not so early that Animate/body is half-created.
setupCharacterLifecycle = function()
    disconnect(Connections.Character)
    add(Connections.Character, LocalPlayer.CharacterRemoving:Connect(function()
        stopEmote("Character removing")
    ...
    add(Connections.Character, LocalPlayer.CharacterAdded:Connect(function()
        ...
        task.spawn(function()
            ...
            if State.Settings.AutoReapplyOnRespawn ~= false then
                task.wait(tonumber(State.Settings.AutoReapplyDelay) or 0.85)
                if stage92EnsureMappingsFromSavedMeta then pcall(stage92EnsureMappingsFromSavedMeta) end
                stage12ApplyAnimateStrict("Safe respawn reapply")
            end
            rebuildFloatingButtons(); rebuildQuickSelector(); updateRuntimeBar()
        end)
    end))
    refreshCharacterReferences(); captureOriginalAnimations()
end

local stage12OldRenderSettings = renderSettings
renderSettings = function()
    stage12OldRenderSettings()
    local sc = nil
    for _, c in ipairs(Gui.Body:GetChildren()) do if c:IsA("ScrollingFrame") then sc = c break end end
    if not sc then return end
    local y = sc.CanvasSize.Y.Offset + 8
    label(sc, "Stage 12 Safe Bundle", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,24), 16, THEME.Text); y = y + 34
    label(sc, "Apply mode forced to Animate-only to avoid avatar reset/mixing. Respawn apply waits for Animate to be ready, not instant 0.25s.", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,44), 12, THEME.Muted); y = y + 52
    button(sc, "REAPPLY CLEAN", UDim2.new(0,12,0,y), UDim2.new(0,130,0,32), function()
        if stage92EnsureMappingsFromSavedMeta then pcall(stage92EnsureMappingsFromSavedMeta) end
        stage12ApplyAnimateStrict("Manual clean reapply")
    end, THEME.Yellow)
    button(sc, "DELAY 0.85s", UDim2.new(0,154,0,y), UDim2.new(0,112,0,32), function()
        State.Settings.AutoReapplyDelay = 0.85; saveData(); renderSettings()
    end, THEME.Cyan)
    button(sc, "DELAY 1.25s", UDim2.new(0,278,0,y), UDim2.new(0,112,0,32), function()
        State.Settings.AutoReapplyDelay = 1.25; saveData(); renderSettings()
    end, THEME.Cyan)
    sc.CanvasSize = UDim2.new(0,0,0,y+70)
end
...
task.defer(function()
    ...
    if State.Settings.AutoApplyOnExecute ~= false then
        task.delay(1.15, function()
            if stage92EnsureMappingsFromSavedMeta then pcall(stage92EnsureMappingsFromSavedMeta) end
            stage12ApplyAnimateStrict("Execute clean apply")
        end)
    end
    if Gui.Main and Gui.Main.Visible then
        if State.CurrentPage == "Bundles" then renderHome("Bundle")
        elseif State.CurrentPage == "Settings" then renderSettings() end
    end
    notify("Stage 12 clean bundle fix loaded", true)
end)

---------------------------------------------------------------------
-- CONTINUATION STAGE 13: FIX POLLUTED BUNDLE MAPPINGS / NO DUPLICATE WALK
-- Root fix for the "jalan trt trt / campur / tubuh kacau" report:
-- older resolver could add fallback container IDs even when real AnimationIds
-- were already found. That can create duplicate WalkAnim/RunAnim children or
-- wrong animation asset IDs. Stage 13 resolves object mappings cleanly and
-- fallback is only used for states that have ZERO real mapping.
---------------------------------------------------------------------

local Stage13 = {
    Version = "13-clean-mapping-no-fallback-pollution",
}

State.Cache.BundleMappingsStrict = State.Cache.BundleMappingsStrict or {}

local STAGE13_FOLDER_BY_STATE = {
    Idle = "idle",
    Walk = "walk",
    Run = "run",
    Jump = "jump",
...
local function stage13FolderToState(folder)
    folder = string.lower(tostring(folder or ""))
    if folder == "idle" then return "Idle" end
    if folder == "walk" then return "Walk" end
    if folder == "run" then return "Run" end
    if folder == "jump" then return "Jump" end
    if folder == "fall" then return "Fall" end
    if folder == "climb" then return "Climb" end
    if folder == "swim" then return "Swim" end
    if folder == "swimidle" then return "SwimIdle" end
    return nil
end

local function stage13CollectObjectMappings(root, out)
    local function walk(obj, path)
        for _, child in ipairs(obj:GetChildren()) do
            local p = path .. "." .. child.Name
            if child:IsA("Animation") then
                local parentName = child.Parent and child.Parent.Name or ""
                local folder = string.lower(parentName)
                local state = stage13FolderToState(folder)
                if not state then
                    state = categorizeAnimation(p)
                    folder = state and (STAGE13_FOLDER_BY_STATE[state] or string.lower(state)) or folder
                end
                local id = normalizeId(child.AnimationId)
                if state and folder ~= "" and id ~= "" then
                    local weights = {}
                    for _, w in ipairs(child:GetChildren()) do
            ...
            if #child:GetChildren() > 0 then walk(child, p) end
        end
    end
    walk(root, root.Name)
end

local function stage13CleanMappings(mappings)
    local byFolder = {}
    local seen = {}
    for _, m in ipairs(mappings or {}) do
        local id = normalizeId(m.Id or m.id or m.AnimationId or m.animationId)
        local folder = string.lower(tostring(m.Folder or ""))
        local state = m.State or stage13FolderToState(folder)
        if folder == "" and state then folder = STAGE13_FOLDER_BY_STATE[state] or string.lower(state) end
        if id ~= "" and folder ~= "" and state then
            local key = folder .. ":" .. tostring(m.Name or "") .. ":" .. id
            if not seen[key] then
                seen[key] = true
                byFolder[folder] = byFolder[folder] or {Folder=folder, State=state, Items={}}
                table.insert(byFolder[folder].Items, {
                    State = state,
                    Folder = folder,
                    Name = tostring(m.Name or ""),
                    Id = id,
                    Weights = type(m.Weights) == "table" and m.Weights or type(m.weights) == "table" and m.weights or {},
                    RealObject = m.RealObject == true,
                })
            end
        end
    end
    ...
    local cleaned = {}
    local order = {"idle", "walk", "run", "jump", "fall", "climb", "swimidle", "swim"}
    for _, folder in ipairs(order) do
        local group = byFolder[folder]
        if group and #group.Items > 0 then
            ...
            if folder == "idle" then
                -- Idle is allowed to have Animation1/Animation2 variants.
                local idleCount = 0
                local usedIds = {}
                for _, m in ipairs(group.Items) do
                    if not usedIds[m.Id] then
                        ...
                        m.Name = (string.lower(m.Name):find("animation", 1, true) and m.Name) or ("Animation" .. tostring(idleCount))
                        table.insert(cleaned, m)
                    end
                end
            else
                -- Movement states must be SINGLE animation only. Duplicate WalkAnim/RunAnim causes jitter/mix.
                local chosen = nil
                local canonical = STAGE13_SINGLE_NAME[folder]
                for _, m in ipairs(group.Items) do
                    if canonical and string.lower(tostring(m.Name)) == string.lower(canonical) then chosen = m break end
                end
                chosen = chosen or group.Items[1]
                chosen.Name = canonical or chosen.Name
                table.insert(cleaned, chosen)
            end
        end
    end
    return cleaned
...
local function stage13ResolveBundleMappings(bundleId)
    bundleId = normalizeId(bundleId)
    if bundleId == "" then return {} end
    ...
    for _, item in ipairs(items) do
        ...
        if assetId ~= "" then
            ...
            if ok and objects then
                for _, obj in ipairs(objects) do
                    stage13CollectObjectMappings(obj, raw)
                    pcall(function() obj:Destroy() end)
                end
            end
        end
    ...
    local cleaned = stage13CleanMappings(raw)

    -- Fallback only for states that have no real object mapping at all.
    local hasState = {}
    for _, m in ipairs(cleaned) do hasState[m.State] = true end
    local typeMap = {[48]="Climb", [50]="Fall", [51]="Idle", [52]="Jump", [53]="Run", [54]="Swim", [55]="Walk"}
    local fallback = {}
    for _, item in ipairs(items) do
        local state = typeMap[tonumber(item.assetType or item.AssetType or item.assetTypeId or item.AssetTypeId)] or categorizeAnimation(item.name or item.Name)
        local id = normalizeId(item.id or item.Id)
        if state and not hasState[state] and id ~= "" then
            local folder = STAGE13_FOLDER_BY_STATE[state] or string.lower(state)
            table.insert(fallback, {State=state, Folder=folder, Name=STAGE13_SINGLE_NAME[folder] or "Animation1", Id=id, Weights={}, RealObject=false})
            hasState[state] = true
        end
    end
    for _, m in ipairs(stage13CleanMappings(fallback)) do table.insert(cleaned, m) end

    State.Cache.BundleMappingsStrict[bundleId] = cleaned
    saveData()
    return cleaned
end

local function stage13MappingsToGroups(mappings)
    local groups = {}
    for _, m in ipairs(stage13CleanMappings(mappings or {})) do
        local folder = string.lower(tostring(m.Folder))
        groups[folder] = groups[folder] or {Folder=folder, State=m.State, Items={}}
        table.insert(groups[folder].Items, m)
    end
    return groups
end

local function stage13ApplyCleanMappings(reason)
    if not stage12RigReady(3.0) then setStatus("Rig not ready", false); return false end
    refreshCharacterReferences()
    local animate = State.Character.Animate
    if not animate then setStatus("Animate not found", false); return false end

    State.CurrentMappings = stage13CleanMappings(State.CurrentMappings or {})
    local groups = stage13MappingsToGroups(State.CurrentMappings)
    local any = false
    for _ in pairs(groups) do any = true break end
    if not any then
        groups = stage12GroupsFromMappingsOrForm()
    end
    ...
    for _, group in pairs(groups) do
        local folder = stage12Folder(animate, group.Folder)
        if folder and group.Items and #group.Items > 0 then
            folders = folders + 1
            stage12ClearFolder(folder)
            for index, m in ipairs(group.Items) do
                local id = normalizeId(m.Id)
                if id ~= "" then
                    local anim = Instance.new("Animation")
                    if string.lower(group.Folder) == "idle" then
                        anim.Name = (string.lower(tostring(m.Name)):find("animation", 1, true) and m.Name) or ("Animation" .. tostring(index))
                    else
                        anim.Name = STAGE13_SINGLE_NAME[string.lower(group.Folder)] or stage12AnimName(group.Folder, m.Name, index)
                    end
                    anim.AnimationId = toAnimationUrl(id)
                    stage12ApplyWeights(anim, m.Weights, (string.lower(group.Folder) == "idle" and #group.Items > 1) and 10 or nil)
                    anim.Parent = folder
    ...
    if changed <= 0 then setStatus("No clean mappings", false); return false end
    stage12RestartAnimateOnly()
    task.delay(0.22, stage12RestartAnimateOnly)
    if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
    saveData()
    setStatus(tostring(reason or "Clean applied") .. " | " .. tostring(changed) .. " anims / " .. tostring(folders) .. " folders", true)
    return true
end

applyCurrentForm = function(name)
    if name then State.LastAppliedName = name end
    if stage13ApplyCleanMappings(name or State.LastAppliedName or "Clean Apply") then return end
    -- fallback only if there are no object mappings
    stage12ApplyAnimateStrict(name or "Fallback Apply")
end

applyBundleFull = function(bundleId, bundleName)
    bundleId = normalizeId(bundleId)
    if bundleId == "" then notify("Invalid bundle", false); return end
    showLoading("Resolving clean mappings...")
    task.spawn(function()
        local mappings = stage13ResolveBundleMappings(bundleId)
        hideLoading()
        if not mappings or #mappings == 0 then
            setStatus("No clean animations found in bundle", false)
            notify("Bundle resolve failed", false)
            return
        end
        State.CurrentMappings = stage13CleanMappings(mappings)
        local form, meta, counts = stage9MappingsToForm(State.CurrentMappings)
        for _, s in ipairs(STATES) do
            State.CurrentForm[s] = form[s] or ""
            State.SlotMeta[s] = meta[s]
            if State.SlotMeta[s] then
        ...
        saveData()
        if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
        local ok = stage13ApplyCleanMappings(State.LastAppliedName)
        if ok then notify("Clean bundle applied | idle variants: " .. tostring(counts.Idle or 0), true) end
    end)
end

-- Override saved BundleId recovery so it does not reload polluted Stage9 cache.
stage92EnsureMappingsFromSavedMeta = function()
    if type(State.CurrentMappings) == "table" and #State.CurrentMappings > 0 then
        State.CurrentMappings = stage13CleanMappings(State.CurrentMappings)
        return true
    end
    local id, name = stage92InferLastBundleId()
    if not id then return false end
    task.spawn(function()
        local mappings = stage13ResolveBundleMappings(id)
        if mappings and #mappings > 0 then
            State.CurrentMappings = stage13CleanMappings(mappings)
            State.LastAppliedName = State.LastAppliedName or name
            if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
            setStatus("Recovered clean mappings from BundleId", true)
        end
    end)
    return true
end

-- Settings quick fix button to clear old polluted caches/snapshots.
local stage13OldRenderSettings = renderSettings
renderSettings = function()
    stage13OldRenderSettings()
    local sc=nil; for _, c in ipairs(Gui.Body:GetChildren()) do if c:IsA("ScrollingFrame") then sc=c break end end
    if not sc then return end
    local y = sc.CanvasSize.Y.Offset + 8
    label(sc, "Stage 13 Mapping Fix", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,24), 16, THEME.Text); y=y+34
    button(sc, "CLEAR OLD MAP CACHE", UDim2.new(0,12,0,y), UDim2.new(0,170,0,32), function()
        State.Cache.BundleMappings = {}
        State.Cache.BundleMappingsStrict = {}
        State.CurrentMappings = {}
        if CAN_SAVE then pcall(function() if isfile(STAGE9_MAPPING_FILE) then delfile(STAGE9_MAPPING_FILE) end end) end
        saveData(); renderSettings(); notify("Old mapping cache cleared. Apply bundle again.", true)
    end, THEME.Red)
    button(sc, "CLEAN REAPPLY", UDim2.new(0,194,0,y), UDim2.new(0,130,0,32), function()
        if stage92EnsureMappingsFromSavedMeta then pcall(stage92EnsureMappingsFromSavedMeta) end
        task.delay(0.35, function() stage13ApplyCleanMappings("Manual clean reapply") end)
    end, THEME.Yellow)
    y=y+42
    label(sc, "If previous versions polluted saved mappings, press CLEAR OLD MAP CACHE once, then apply the bundle again. Non-idle states are now forced single animation only.", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,54), 12, THEME.Muted)
    sc.CanvasSize = UDim2.new(0,0,0,y+80)
end
...
task.defer(function()
    ...
    notify("Stage 13 clean mapping fix loaded", true)
end)
