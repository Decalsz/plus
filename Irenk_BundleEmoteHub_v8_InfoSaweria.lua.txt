--[[
    Irenk Bundle + Emote Hub v8 - Info Saweria
...
local function loadData()
    ...
    if type(data.EmoteSpeed) == "number" then EmoteSpeed = math.clamp(data.EmoteSpeed, 0.1, 5) end
...
local function showInfoModal(titleText, bodyText, imageId, actions)
    closeInfoModal()
...
local function renderItemCard(parent, item, index, kind)
    ...
    makeButton(card, "INFO", UDim2.new(0,110,1,-36), UDim2.new(0,58,0,26), function()
        ...
        showInfoModal(name, body, imgId, actions)
    end, Theme.Cyan)
...
renderHome = function(kind)
    ...
    if #list == 0 then
        ...
        if hasNext then
            makeButton(scroll, "LOAD MORE", UDim2.new(0,12,0,rows*142+16), UDim2.new(0,180,0,34), function()
...
renderSettings = function()
    ...
    local speeds = {{"1x",1},{"1.5x",1.5},{"2x",2},{"3x",3}}
    local x = 12
    for _, sp in ipairs(speeds) do
        makeButton(Body, sp[1], UDim2.new(0,x,0,120), UDim2.new(0,70,0,34), function() EmoteSpeed=sp[2]; if CurrentEmoteTrack then pcall(function() CurrentEmoteTrack:AdjustSpeed(EmoteSpeed) end) end; saveData(); renderSettings() end, EmoteSpeed==sp[2] and Theme.Green or Theme.Card)
        x = x + 80
    end
    makeButton(Body, EmoteLoop and "LOOP: ON" or "LOOP: OFF", UDim2.new(0,12,0,170), UDim2.new(0,130,0,34), function() EmoteLoop=not EmoteLoop; if CurrentEmoteTrack then pcall(function() CurrentEmoteTrack.Looped=EmoteLoop end) end; saveData(); renderSettings() end, EmoteLoop and Theme.Green or Theme.Card)
    makeButton(Body, MoveWhileEmote and "MOVE: ON" or "MOVE: OFF", UDim2.new(0,154,0,170), UDim2.new(0,130,0,34), function() MoveWhileEmote=not MoveWhileEmote; saveData(); renderSettings() end, MoveWhileEmote and Theme.Green or Theme.Card)
    makeLabel(Body, "Modal background transparency", UDim2.new(0,12,0,220), UDim2.new(1,-24,0,24), 13, Theme.Muted)
    local dims = {{"25%",0.25},{"45%",0.45},{"65%",0.65},{"80%",0.80}}
    x = 12
    for _, d in ipairs(dims) do
        makeButton(Body, d[1], UDim2.new(0,x,0,250), UDim2.new(0,70,0,34), function() ModalDimTransparency=d[2]; saveData(); renderSettings() end, math.abs(ModalDimTransparency-d[2])<0.01 and Theme.Green or Theme.Card)
        x = x + 80
    end
    makeLabel(Body, "Apply Method", UDim2.new(0,12,0,300), UDim2.new(0,160,0,24), 13, Theme.Muted)
    makeButton(Body, "ANIMATE", UDim2.new(0,12,0,330), UDim2.new(0,100,0,34), function() ApplyMethod="Animate"; saveData(); renderSettings() end, ApplyMethod=="Animate" and Theme.Green or Theme.Card)
    makeButton(Body, "DESCRIPTION", UDim2.new(0,124,0,330), UDim2.new(0,130,0,34), function() ApplyMethod="Description"; saveData(); renderSettings() end, ApplyMethod=="Description" and Theme.Green or Theme.Card)
    makeButton(Body, "BOTH", UDim2.new(0,266,0,330), UDim2.new(0,90,0,34), function() ApplyMethod="Both"; saveData(); renderSettings() end, ApplyMethod=="Both" and Theme.Green or Theme.Card)
    makeButton(Body, AutoLoad and "AUTOLOAD: ON" or "AUTOLOAD: OFF", UDim2.new(0,12,0,390), UDim2.new(0,150,0,34), function() AutoLoad=not AutoLoad; saveData(); renderSettings() end, AutoLoad and Theme.Green or Theme.Card)
    makeButton(Body, "STOP EMOTE", UDim2.new(0,174,0,390), UDim2.new(0,120,0,34), stopEmote, Theme.Red)
    makeButton(Body, "RESET ORIGINAL", UDim2.new(0,306,0,390), UDim2.new(0,140,0,34), restoreOriginal, Theme.Yellow)
end
...
task.spawn(function()
    ...
    if AutoLoad and hasAny then
        ...
        if ok then status("Popular bundles loaded", true) else status("Search bundles or emotes.", true) end
    end
end)
